...
- **Provided Assets: (for the example setup) **
    ...
    - `jobs/ingest_yfinance_to_kafka.py` - Producer ingestion for market events.
...
- **Provided Assets:**
    ...
    - `jobs/spark_kafka_to_bronze.py` - Kafka to Bronze transformation entrypoint.
...
| `jobs/ingest_yfinance_to_kafka.py` | Market data producer ingestion |
| `jobs/spark_session_factory.py` | Skeleton - SparkSession factory |
| `jobs/spark_kafka_to_bronze.py` | Kafka to Bronze Spark pipeline |
